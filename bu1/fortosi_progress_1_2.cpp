...
fortosi_progress_1_2::fortosi_progress_1_2(QWidget *parent,QTcpSocket *client,QString fid)
    : QDialog(parent)
{
	this->client=client;
	ui.setupUi(this);
...
void fortosi_progress_1_2::finalize()
{
		...
		out.setVersion(QDataStream::Qt_4_1);
		QString req_type="SF";
		out << quint16(0)<<req_type<<customer<<ui.tableWidget->item(i,0)->text()<<ui.labelWeight->text()<<fortosi_id;
		out.device()->seek(0);
		out<<quint16(block.size()-sizeof(quint16));
		client->write(block);
		}
