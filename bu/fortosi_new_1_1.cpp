...
fortosi_new_1_1::fortosi_new_1_1(QWidget *parent,QString customer,QString car1,QString car2)
    : QDialog(parent)
{
...
{
	//QHostAddress addr("192.168.2.188");
		...
		//{
			...
			//QDataStream out(&block,QIODevice::WriteOnly);
			//out.setVersion(QDataStream::Qt_4_1);
			//out<<quint16(0)<<customer<<car1<<car2<<ui.tableWidget->item(i,0)->text();
			//out.device()->seek(0);
			//out<<quint16(block.size()-sizeof(quint16));
			//client->write(block);
		//}
...
{
	QHostAddress addr("192.168.0.188");
	...
	for (int i=0;i<r;++i)
	{
		...
		out2<<quint16(0)<<customer<<car1<<car2<<ui.tableWidget->item(i,0)->text();
		out2.device()->seek(0);
